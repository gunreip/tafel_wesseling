...
Route::get('/healthz', function (\Illuminate\Http\Request $request) {
    ...
    if ($deep) {
        try { DB::connection()->getPdo(); $checks['db'] = 'ok'; }
        catch (\Throwable $e) { $checks['db'] = 'fail'; $status = 'fail'; }

        try {
            $key = 'healthz:ping';
            Cache::put($key, 'pong', 5);
            $checks['cache'] = Cache::get($key) === 'pong' ? 'ok' : 'fail';
            if ($checks['cache'] !== 'ok') { $status = 'fail'; }
        } catch (\Throwable $e) { $checks['cache'] = 'fail'; $status = 'fail'; }
    }
